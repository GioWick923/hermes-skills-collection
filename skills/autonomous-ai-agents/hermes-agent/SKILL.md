...
**`hermes config set` raises `PermissionError: [WinError 5] Acceso denegado` while Desktop/TUI is running.**
This can happen on Windows when the config file is momentarily locked during the CLI's atomic replace.
First retry the command. If it repeats and the user explicitly asked to edit profile config, use a small Python/YAML script to load the target `config.yaml`, modify only the intended keys, and write it back with `Path.write_text(...)`; then verify with `hermes profile list`, `hermes --profile <name> tools list`, or by reading the changed keys. Do not use this workaround for secrets; secrets belong in `.env`/`hermes auth`.
