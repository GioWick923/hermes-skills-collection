...
Isolates APPDATA/LOCALAPPDATA to a temp dir so the real ledger is untouched.
Exercises: observe, reflect, prune, recall, fold, status, drop, view.
AD-HOC verification, not a committed suite; run with:
    python scripts/leager_verify.py
Exits 0 on pass, 1 on failure.
"""
import os, sys, subprocess, tempfile, json, shutil

SKILL_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ENGINE = os.path.join(SKILL_DIR, "scripts", "leager.py")
...
def main():
    ...
    failures = []
    try:
        rc, out, err = run(tmp, "observe", "--content", "obs A crit", "--relevance", "critical")
        ...
        rc, out, err = run(tmp, "reflect", "--content", "ref durable 1", "--supports", oid_a, "--supports", oid_a)
        ...
        assert rc == 0 and "OBSERVATION [dropped]" in out, f"recall-dropped: {out} {err}"
        ...
        assert oid_a not in out and rid in out and oid_b in out and oid_c in out, "fold projection wrong"

        rc, out, err = run(tmp, "status")
        assert rc == 0 and "observations active   : 2" in out and "observations recorded : 3" in out and "reflections           : 1" in out, f"status: {out}"
        ...
        assert rc == 0 and "invalid source id ignored" in err, f"invalid-source: {out} {err}"

        lf = os.path.join(ledger_dir, "ledger.jsonl")
        assert os.path.exists(lf), "ledger file missing"
    ...
    if failures:
        ...
        for f in failures:
            print("  -", f)
        sys.exit(1)
    print("VERIFY: PASS -- all checks on leager.py passed (ad-hoc, isolated env).")
