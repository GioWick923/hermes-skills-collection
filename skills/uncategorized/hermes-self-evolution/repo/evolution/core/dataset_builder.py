...
class SyntheticDatasetBuilder:
    ...
    def generate(
        ...
        # Parse the generated test cases (robust to markdown fences / prose)
        cases_raw = None
        raw = (result.test_cases or "").strip()
        ...
        for attempt in (raw,):
            try:
                ...
                try:
                    ...
                    cases_raw = repair_loads(attempt)
                    break
                except Exception:
                    try:
                        ...
                        if match:
                            ...
                            cases_raw = repair_loads(match.group())
                            break
                    except Exception:
                        pass
        if cases_raw is None:
            raise ValueError(f"Could not parse test cases from LLM output: {result.test_cases[:300]}")

        if not isinstance(cases_raw, list):
            # some models wrap in a dict: {"test_cases": [...]}
            if isinstance(cases_raw, dict):
                for k, v in cases_raw.items():
                    if isinstance(v, list):
                        ...
                        break
            if not isinstance(cases_raw, list):
                raise ValueError(f"Parsed JSON is not a list: {str(cases_raw)[:200]}")
