...
- On Linux (this fork), auto-install via apt + pip --user when running as root.
  On other Linux setups it prints exact commands for the user to run.
...
def _linux_apt_packages(missing: list[str]) -> list[str]:
    pkgs: list[str] = []
    for bin_name in missing:
        if bin_name in ("ffmpeg", "ffprobe") and "ffmpeg" not in pkgs:
            pkgs.append("ffmpeg")
        elif bin_name == "yt-dlp":
            # yt-dlp is best via pip/pipx — the apt version tends to lag.
            pass
    return pkgs
...
def _install_macos(missing: list[str]) -> tuple[bool, str]:
    if _which("brew") is None:
        return False, (
            ...
            "Or install manually: `brew install ffmpeg yt-dlp`"
        )
    pkgs: list[str] = []
    for b in missing:
...
def _install_linux(missing: list[str]) -> tuple[bool, str]:
    """On Linux, try apt for ffmpeg, pip --user for yt-dlp.

    When running as root we drop the sudo prefix. When not, we print the
    ...
    apt_pkgs = _linux_apt_packages(missing)
    yt_dlp_missing = "yt-dlp" in missing

    if apt_pkgs:
        cmd = [*apt_prefix, "apt-get", "install", "-y", *apt_pkgs]
        print(f"[setup] running: {' '.join(cmd)}", file=sys.stderr)
        result = subprocess.run(cmd)
        if result.returncode != 0:
            return False, f"apt-get install failed with exit code {result.returncode}"

    if yt_dlp_missing:
        # pip --user keeps it off the system Python and works without pipx.
        cmd = [sys.executable, "-m", "pip", "install", "--user", "yt-dlp"]
        print(f"[setup] running: {' '.join(cmd)}", file=sys.stderr)
        result = subprocess.run(cmd)
        if result.returncode != 0:
            return False, f"pip install yt-dlp failed with exit code {result.returncode}"
        # Ensure ~/.local/bin is on PATH for this session and future shells.
        local_bin = str(Path.home() / ".local" / "bin")
        if local_bin not in os.environ.get("PATH", ""):
            os.environ["PATH"] = f"{local_bin}:{os.environ.get('PATH', '')}"
            print(
                f"[setup] note: added {local_bin} to PATH for this process; "
                f"add it to your shell rc to persist.",
                file=sys.stderr,
            )

    still_missing = _check_binaries()
    if still_missing:
        return False, f"still missing after install: {', '.join(still_missing)}"
    if needs_sudo:
        return True, f"installed via apt/pip (with sudo). add ~/.local/bin to PATH."
    return True, "installed via apt/pip"
...
def _install_hint_linux(missing: list[str]) -> str:
    ...
    if "yt-dlp" in missing:
        hints.append("`pip install --user yt-dlp` (or `pipx install yt-dlp`)")
