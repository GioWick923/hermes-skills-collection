...
license: Apache-2.0
description: Use when fixing OAuth redirect_uri errors on portless URLs.
metadata:
  hermes:
    tags: [oauth, auth, localhost, portless, redirect-uri]
    related_skills: [portless]
---
...
- Setting up OAuth (Google, Apple, Microsoft, Facebook, GitHub) for local dev
- Hitting `redirect_uri_mismatch` / `invalid redirect` on `.localhost` subdomains
- Configuring sign-in providers for portless dev URLs
- Needing a valid TLD because a provider rejects `.localhost`
