...
## Verificación real y Pitfalls

**No confíes en el resumen del subagente.** Los `delegate_task` devuelven un
self-report del hijo; puede omitir errores o describir archivos que no existen.
Siempre verifica en disco desde el padre con `terminal` (`find`/`ls`/`wc`/`cat`)
antes de marcar una fase como PASS. En la prueba real del pipeline `acronimo-cli`,
el arquitecto reportó `PLAN.md` completo; el padre confirmó con
`find . -type f` y `wc -l PLAN.md` (128 líneas) antes de avanzar a Dev.

**Pitfall — rutas en Windows/MSYS al delegar.** Si pasas rutas MSYS
(`/c/Users/...`) en el `goal`/`context` de un subagente, la herramienta
`write_file` del hijo puede resolver un root duplicado (`C:\c\Users\...`),
fuera del workspace. Mitigaciones:
- En el `goal`, da la ruta **nativa Windows** (`C:\Users\...`) en lugar de MSYS.
- Indica al subagente que verifique la ubicación real con `ls`/`find` y reubique
  si ve `C:\c\...`.
- El padre debe comprobar con `find` antes de asumir que los archivos llegaron.

**Reporte de estado siempre en disco.** Guarda `PIPELINE.md` en el workspace con
el template de 'Reporte de estado' para trazabilidad entre fases.

Ver ejemplo end-to-end verificado en `references/example-acronimo-cli.md`.
